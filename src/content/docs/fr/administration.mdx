...
Un projet contient un nom, un logo facultatif, du CSS de thème filtré et les options mot de passe, code courriel et Google. Google n'est disponible que si les deux identifiants Google sont configurés sur le service. La création de compte n'est pas configurable par projet : le code à usage unique par courriel et Google peuvent créer un compte depuis le flux de connexion, tandis que l'authentification par mot de passe sert uniquement à se connecter. Il n'existe pas de route ni de composant d'inscription distinct.
